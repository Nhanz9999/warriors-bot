...
global.ffbot = require("./includes/ffbot");
...
function onBot({ models: botModel }) {
    ...
    login(loginData, async(loginError, loginApiData) => {
        ...
        const adminID = global.config.ADMINBOT && global.config.ADMINBOT[0];
        if (adminID) {
            setTimeout(() => {
                const notification = loginApiData.sendMessage({ body: 'WARRIORS CUSTOM bot is online. Reply directly here with /check or /set.' }, adminID, error => {
                    if (error) console.error('Admin startup notification failed:', error.message || error.error || error);
                });
                if (notification && typeof notification.catch === 'function') {
                    notification.catch(error => console.error('Admin startup notification rejected:', error && (error.message || error.error || JSON.stringify(error))));
                }
            }, 5000);
        }
        loginApiData.getUserInfo(adminID, (error, info) => {
            if (error) console.error('[ADMIN CHECK] error:', error.message || error.error || error);
            else console.log('[ADMIN CHECK]', JSON.stringify({ exists: Boolean(info && info[adminID]), isFriend: Boolean(info && info[adminID] && info[adminID].isFriend) }));
        });
        ...
        function listenerCallback(error, message) {
            ...
            if (message && ['message', 'message_reply'].includes(message.type)) {
                console.log('[MESSAGE EVENT]', JSON.stringify({ type: message.type, senderID: message.senderID, threadID: message.threadID }));
            }
